...
(function() {
    ...
    this.commands = {
        ...
        "Tab": function(editor) {
        	...
        	if(editor && editor.completer && editor.completer.completions && editor.completer.completions && 
        	   editor.completer.completions.filtered && editor.completer.completions.filtered.length === 1) {
...
Autocomplete.startCommand = {
    ...
    bindKey: "Ctrl-Space|Ctrl-Shift-Space|Alt-Space|Period|\\."
};
...
(function(){
    this.setFilter = function(str) {
		...
		var maxResults = parseInt($('input#ac-max-results').val());

		matches = matches.length > maxResults ? matches.slice(0, maxResults) : matches;
    ...
    this.filterCompletions = function(items, needle) {
        ...
        loop: for (var i = 0, item; item = items[i]; i++) {
            ...
            if (!caption || (!includeLocalAC && item.meta === 'local') || (caption && caption.length && caption.length > 200)) continue;
            var lastIndex = -1;
            var matchMask = 0;
            var penalty = 0;
            var index, distance;
...
var doLiveAutocomplete = function(e) {
    ...
    if (e.command.name === "backspace") {
        ...
        if (prefix && !hasCompleter) {
        	...
        	var altToken  = editor.session.getTokenAt(cursorPos.row, cursorPos.column);

        	if(altToken && altToken.value === ".") {
        	
				var prevToken = null;
				if(altToken.start) {
					prevToken = editor.session.getTokenAt(cursorPos.row, altToken.start);
				}

		        if(prevToken !== null && prevToken && prevToken.value && (prevToken.type === 'rparen' || apexLang[prevToken.value.toLowerCase()] || (classVariableMap[prevToken.value] && apexLang[classVariableMap[prevToken.value].toLowerCase()]))) {
